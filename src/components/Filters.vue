...
export default {
  ...
  methods: {
    changeUploadDateMobile() {
      ...
      if (upldate == "any") this.uploadDate = "";
      else {
        let backHours = 0;
        if (upldate == "today") backHours = 1000 * 60 * 60 * 24;
        else if (upldate == "week") backHours = 1000 * 60 * 60 * 24 * 7;
        else if (upldate == "month") backHours = 1000 * 60 * 60 * 24 * 7 * 30;

        var time = new Date().getTime();
        var date = new Date(time - backHours)
          .toISOString()
          .replaceAll(":", "%3A");
    ...
    changeUploadDate(e) {
      ...
      if (alreadySelected) {
        ...
        let backHours = 0;
        if (e.target.innerText == "Last Hour") backHours = 1000 * 60 * 60;
        else if (e.target.innerText == "Today") backHours = 1000 * 60 * 60 * 24;
    ...
    changeType(e) {
      ...
      if (alreadySelected) {
        ...
        e.target.classList.add("selected");
        if (e.target.innerText == "Video") this.type = "video";
        else if (e.target.innerText == "Channel") this.type = "channel";
        else if (e.target.innerText == "Playlist") this.type = "playlist";
      }
    },
    changeSortBy(e) {
      let alreadySelected = e.target.classList.contains("selected");
      ...
      if (alreadySelected) {
        ...
        e.target.classList.add("selected");
        if (e.target.innerText == "Relevance") this.sortBy = "relevance";
        else if (e.target.innerText == "Upload Date") this.sortBy = "date";
        else if (e.target.innerText == "View Count") this.sortBy = "viewCount";
        else if (e.target.innerText == "Rating") this.sortBy = "rating";
      }
    },
    slide() {
